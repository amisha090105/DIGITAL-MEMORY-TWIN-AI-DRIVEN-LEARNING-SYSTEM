...
"""
Main application entry point for MULRAG - Hackathon MVP Version.
Authentication removed for single-user demo mode.
"""
...
@asynccontextmanager
async def lifespan(app: FastAPI):
    """Application lifespan manager (No Auth Version)."""
    
    logger.info("Starting MULRAG (Hackathon Mode)...")
    ...
    try:
        ...
        logger.info("MULRAG started successfully (Auth Disabled)")
        ...
        logger.info("Shutting down MULRAG...")
        db_manager.close()
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title=settings.APP_NAME + " - Hackathon",
        version=settings.VERSION,
        description="Single-user Multi-Agent RAG System (Auth Removed)",
        lifespan=lifespan
    )
    ...
    # Include routers (sessions, chat, upload etc.)
    include_routers(app)
    
    @app.get("/", response_class=HTMLResponse)
    async def root():
        return {
            "message": "MULRAG Hackathon Mode Running",
            "auth": "disabled",
            "version": settings.VERSION
        }
